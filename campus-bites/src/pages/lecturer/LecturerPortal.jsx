...
import { FilterPill, EmptyState, PrimaryButton, GlassCard, StatusChip } from '../../components/ui';

const LecturerPortal = () => {
    const { user, token, logout } = useAuth();
    const { cartItems, addToCart, updateQuantity, clearCart, cartTotal: ctxCartTotal, cartCount: ctxCartCount } = useCart();
    ...
    const taxAmount = Math.round(ctxCartTotal * 0.05);
    const finalTotal = ctxCartTotal + taxAmount;
                ...
                handler: async (response) => {
                    const verify = await fetch(`${API_URL}/api/orders/verify`, {
                        ...
                        body: JSON.stringify({
                            ...
                            orderData: { items: cartItems.map(i => ({ product: i._id, quantity: i.quantity, price: i.price })), totalAmount: finalTotal, deliveryType: 'cabin', cabinNumber: user.cabinNumber, pickupTime: 'Cabin Delivery' }
                        })
                    });
                    if (verify.ok) { clearCart(); toast.success(`Order placed! Delivering to Cabin ${user.cabinNumber}`); setTab('orders'); }
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#0D0D0D', color: 'white', paddingBottom: 100 }}>
            <style>{`
                * { box-sizing: border-box; }
                ...
                .lec-search { width:100%;padding:0.85rem 1rem 0.85rem 2.8rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:0.85rem;color:white;font-size:0.95rem;outline:none;transition:all 0.3s; }
                .lec-search:focus { border-color:#E23744;background:rgba(226,55,68,0.05); }
                .lec-search::placeholder { color:#8B949E; }
                .cat-btn { padding:0.5rem 1.1rem;border-radius:2rem;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);color:#9CA3AF;font-size:0.82rem;font-weight:500;cursor:pointer;transition:all 0.2s;white-space:nowrap; }
                ...
                .add-btn { width:100%;padding:0.6rem;background:linear-gradient(135deg,#E23744,#DC2626);border:none;color:white;border-radius:0.6rem;cursor:pointer;font-size:0.8rem;font-weight:600;transition:all 0.2s; }
                .add-btn:hover { transform:translateY(-1px);box-shadow:0 4px 15px rgba(226,55,68,0.3); }
                .checkout-btn { width:100%;padding:1rem;background:linear-gradient(135deg,#E23744,#DC2626);border:none;color:white;border-radius:1rem;font-size:1.05rem;font-weight:700;cursor:pointer;box-shadow:0 10px 30px rgba(226,55,68,0.3);display:flex;align-items:center;justify-content:center;gap:8px;transition:all 0.3s; }
                ...
                .nav-item { flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 0;border:none;background:transparent;cursor:pointer;color:#9CA3AF;font-size:10px;font-weight:500;transition:all 0.3s; }
            ...
            <div className="glass-panel" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
                ...
                <button onClick={() => { logout(); navigate('/lecturer'); }} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: '#9CA3AF', padding: '7px 12px', borderRadius: 10, cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                {tab === 'menu' && (
                    <div>
                        <div style={{ position: 'relative', marginBottom: 12 }}>
                            <Search size={16} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#8B949E' }} />
                            <input className="lec-search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search food..." />
                        </div>
                        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 8, marginBottom: 16 }}>
                            {categories.map(c => <FilterPill key={c} active={category === c} onClick={() => setCategory(c)}>{c}</FilterPill>)}
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
                            {filtered.map((p, idx) => {
                                const inCart = cartItems.find(i => i._id === p._id);
                                return (
                                        ...
                                        <div style={{ padding: '10px 12px 12px' }}>
                                            ...
                                            {inCart ? (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    <button className="qty-btn" onClick={() => updateQuantity(p._id, -1)} style={{ background: 'rgba(226,55,68,0.15)', color: '#E23744' }}><Minus size={14} /></button>
                                                    <span style={{ fontWeight: 700, flex: 1, textAlign: 'center' }}>{inCart.quantity}</span>
                                                    <button className="qty-btn" onClick={() => updateQuantity(p._id, 1)} style={{ background: '#E23744', color: 'white' }}><Plus size={14} /></button>
                                                </div>
                                            ) : (
                        ...
                        {filtered.length === 0 && <EmptyState icon={Search} title="No items found" />}
                    </div>
                )}
                    ...
                    <div>
                        ...
                        {cartItems.length === 0 ? (
                            <EmptyState icon={ShoppingBag} title="Your cart is empty">
                                <PrimaryButton onClick={() => setTab('menu')} style={{ marginTop: 16 }}>Browse Menu</PrimaryButton>
                            </EmptyState>
                        ) : (
                            <>
                                {/* Cabin Info */}
                                <div className="lec-card" style={{ padding: '14px 16px', marginBottom: 16, background: 'rgba(226,55,68,0.07)', borderColor: 'rgba(226,55,68,0.2)' }}>
                                    <div style={{ fontSize: 12, color: '#8B949E', marginBottom: 4 }}>🚪 Delivering to</div>
                                    <div style={{ fontWeight: 700, fontSize: 17, color: '#E23744' }}>Cabin {user?.cabinNumber}</div>
                                    {user?.department && <div style={{ fontSize: 12, color: '#8B949E', marginTop: 2 }}>{user.department}</div>}
                                </div>
                                    ...
                                    {cartItems.map(item => (
                                        <div key={item._id} className="lec-card" style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                                <button className="qty-btn" onClick={() => updateQuantity(item._id, -1)} style={{ background: 'rgba(255,255,255,0.07)', color: 'white' }}><Minus size={14} /></button>
                                                <span style={{ fontWeight: 700, width: 20, textAlign: 'center' }}>{item.quantity}</span>
                                                <button className="qty-btn" onClick={() => updateQuantity(item._id, 1)} style={{ background: '#E23744', color: 'white' }}><Plus size={14} /></button>
                                            </div>
                                            <div style={{ fontWeight: 700, fontSize: 14, color: 'white', minWidth: 48, textAlign: 'right', flexShrink: 0 }}>₹{item.price * item.quantity}</div>
                                        </div>
                                    ))}
                                </div>
                                <div className="lec-card" style={{ padding: 20 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, color: '#8B949E', fontSize: 14 }}>
                                        <span>Subtotal ({ctxCartCount} items)</span><span>₹{ctxCartTotal}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, color: '#8B949E', fontSize: 14 }}>
                                    ...
                                    return (
                                        <div key={order._id} className="lec-card" style={{ padding: 16 }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                                <div style={{ fontSize: 12, color: '#8B949E' }}>{new Date(order.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</div>
                                                <StatusChip status={order.status} />
                                            </div>
                                            {order.cabinNumber && <div style={{ color: '#E23744', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>🚪 Cabin · {order.cabinNumber}</div>}
                                            <div style={{ color: '#8B949E', fontSize: 13, marginBottom: 8 }}>{order.items?.length} item{order.items?.length !== 1 ? 's' : ''}</div>
            ...
            <nav style={{ position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)', width: '90%', maxWidth: 400, zIndex: 1000 }}>
                <div className="glass-panel" style={{ borderRadius: 24, padding: '12px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
                    {[
                        ...
                        { key: 'cart', icon: ShoppingBag, label: `Cart${ctxCartCount > 0 ? ` (${ctxCartCount})` : ''}`, badge: ctxCartCount },
